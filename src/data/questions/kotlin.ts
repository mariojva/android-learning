...
export const KOTLIN_QUESTIONS: Question[] = [
  {
    ...
    stage: "implement",
    ownership: "implement",
    concepts: ["read-only-collections", "val-var", "interfaces"],
    estimatedMinutes: 10,
...
fun displayName(profile: Profile): String {
  ...
  },
  {
    ...
    stage: "implement",
    ownership: "implement",
    concepts: ["sealed-types", "ui-state"],
    estimatedMinutes: 18,
...
updated.items.add(Item("extra"))`,
    readingPrompts: [
      ...
      },
      {
        ...
        expert:
          "`MutableList` is unstable to the Compose compiler. Since Kotlin 2.0.20 strong skipping is on by default, so a composable taking `Cart` is still *skippable* — but an unstable parameter is compared by instance identity (`===`) rather than by `equals`. So it skips only when the very same `Cart` instance is passed again. Rebuild the cart, or `copy()` it, and it recomposes even if every field is equal. `ImmutableList` or `@Immutable` on the class restores `equals` comparison, which is what makes an equal-but-new instance skip.",
        keywords: ["stable", "identity", "equals", "recompose", "immutable"],
      },
    ],
    solution: {
      mentalModel:
...
val second = items
  ...
  },
  {
    ...
    stage: "implement",
    ownership: "implement",
    concepts: ["collection-operators", "context"],
    estimatedMinutes: 12,
...
data class User(val name: String, val id: String)`,
  ...
  },
  {
    ...
    stage: "predict",
    ownership: "improve",
    concepts: ["equality", "references", "read-only-collections"],
    estimatedMinutes: 14,
    completedCount: 5200,
    introducedInWeek: 2,
    symptom:
